...
fig = px.bar(df, x="PIPELINE NAME", y="CREDITS CONSUMED", title='Credits Consumed by Pipelines')
...
my_cur.execute("SELECT PIPELINE_START_TIME,CREDITS_CONSUMED_FOR_PIPELINE_EXECUTION FROM SNOWVIEW_AUDIT_HISTORY WHERE PIPELINE_START_TIME <='2022-12-31'")
res=my_cur.fetchall()
...
#fig = px.line(df, x='PIPELINE START TIME', y='CREDITS CONSUMED',title='Credits Consumed over Time Period')
